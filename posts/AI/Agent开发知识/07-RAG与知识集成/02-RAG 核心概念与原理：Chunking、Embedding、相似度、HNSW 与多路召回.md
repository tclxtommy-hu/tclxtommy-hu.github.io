...
本文以知识库为主线展开讲解，记忆相关工程落地细节见 [Memory Recall：个人记忆召回是什么、如何实现](../04-记忆系统/02-Memory-Recall.md)。

### 1.2、RAG 的流水线：离线存、在线搜

RAG 的流水线分两段。以下以知识库场景为例展开，记忆检索架构类似，差异主要在存取策略，详见 [Memory Recall](../04-记忆系统/02-Memory-Recall.md)。
...
用户问「我上次提到的那个 Go 项目怎么部署？」——答案不在知识库里，而在这个用户的历史记忆或对话上下文中。用户问「支付服务依赖哪些中间件？」——答案往往在实体关系图的多跳边上，而不是某一段语义近邻文档里。

知识库用向量检索，记忆用 Memory Recall，关联结构用 Graph Recall，结构化数据用 SQL 查询——不同的数据类型需要不同的检索方式。召回层该用哪几路、怎么组合，取决于业务场景，没有固定答案。Graph Recall 的定义、建图、K-hop/路径/社区检索与实现见 [Graph Recall](04-Graph-Recall.md)；Memory Recall 见 [Memory Recall](../04-记忆系统/02-Memory-Recall.md)。多路检索各跑各的，各自返回一批结果和分数。但分数怎么合并？直接加行不行？
