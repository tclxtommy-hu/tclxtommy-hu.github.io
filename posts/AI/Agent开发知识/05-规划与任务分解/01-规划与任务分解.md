...
下图是 `planningAgent.ts` 中 `StateGraph` 的实际拓扑，直观展示"规划 → 分步执行 → 失败重规划 → 汇总"的闭环：
...
> `MAX_REPLAN=3` 防止重规划死循环；`results` 用 reducer 累加，重规划后已完成结果不丢失。
...
| API | 作用 | 代码位置 |
|-----|------|---------|
| `Annotation.Root({...})` | 声明整个图的 State schema | `planningAgent.ts:72` |
| `Annotation<T>({ reducer, default })` | 定义单个字段 + reducer（多步结果如何合并） | `planningAgent.ts:73-83` |
...
| API | 作用 | 代码位置 |
|-----|------|---------|
| `new StateGraph(stateSchema)` | 创建状态图实例 | `planningAgent.ts:205` |
| `.addNode(name, fn)` | 注册节点：`name` 是图里标识，`fn` 是业务函数 | `planningAgent.ts:206-209` |
...
| API | 作用 | 代码位置 |
|-----|------|---------|
| `.addEdge(from, to)` | 固定边：`from` 执行完必去 `to` | `planningAgent.ts:210-211,217-218` |
| `.addConditionalEdges(from, router, mapping)` | 条件边：`router(state)` 返回字符串 key，`mapping` 决定去哪个节点 | `planningAgent.ts:212-216` |
| `START` / `END` | 特殊节点：图入口 / 图出口常量 | `planningAgent.ts:210,218` |
...
| API | 作用 | 代码位置 |
|-----|------|---------|
| `.compile()` | 把图定义编译成可执行应用（之后不可改） | `planningAgent.ts:219` |
| `.invoke(input)` | 阻塞式运行整条图，返回最终 state | `planningAgent.ts:249` |

> `.compile()` 之前是「设计期」，之后是「运行期」。可选参数如 `interrupt_before` / `interrupt_after` 可插入人工审批节点（Human-in-the-Loop），本 demo 未启用，故全自动。

#### 编码量直觉
> 「图的拓扑 + 路由逻辑 + 状态 schema」≈ 15 行声明式代码（`205-219`），换来的却是一个可自愈、可循环、可兜底的完整 Agent 流程。**这部分是 LangGraph 的核心价值：把流程控制从你手里接管过来声明式编排，而每个节点内的智能仍交给 LLM + prompt。**
...
|------|------|---------------------|-----------|
| **① 图拓扑** | 节点怎么连、有没有环、几条分支 | Plan-and-Execute 单线 + 一个重规划环 | ReAct 单循环 / 多 Agent 协作 / Map-Reduce 并行… |
| **② State 字段 + reducer** | 数据怎么流、怎么累积 | `plan/results/stepIndex/replanCount` + `results` 累加 | 完全不同的字段集和合并规则 |
...
```mermaid
flowchart TD
    S([START]) --> planner --> executor --> route{route}
    route -->|继续| executor
    route -->|失败| replanner --> executor
...
```ts
// 即 planningAgent.ts:205-219，本 demo 实际代码
export const planningGraph = new StateGraph(PlanState)
  ...
  .addEdge("replanner", "executor")   // 重规划后回执行器 → 自愈循环
